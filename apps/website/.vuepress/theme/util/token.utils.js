...
// See website/theme/web-components/token-table.js for usage.

import { html } from 'lit-html';
import { default as tokenData } from '@cds/core/src/styles/tokens/generated/tokens.json';

export function getTokenTable(key) {
  return html` <table class="token-table">
...
function setNumberToken(token) {
  token.formattedValue = html` <div cds-layout="vertical gap:md">
    ${token.alias ? html`<p cds-text="secondary">${token.alias}</p>` : ''}
    <p cds-text="secondary">Web: <span cds-text="code">${(token.value / 20).toFixed(2)}rem</span></p>
...
function setBorderRadiusToken(token) {
  if (token.value !== '50%') {
    setNumberToken(token);
  }
...
function setBorderWidthToken(token) {
  setNumberToken(token);
  token.demo = html`<div
