...
The `aria-live` attributes should never be used within a Core component. Due to
the risk of triggering to many announcements, the use of `aria-live` must be
managed by the application. Application developers can use `aria-live`
...
the light DOM and projected into the component slot. The label element would exist
