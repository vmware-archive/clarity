...
- Core Web Components are built using [lit](https://lit.dev)
...
from `@cds/core/internal` as this provides the same lit decorator with
...
the lit decorator by adding our preferred defaults.
