...
this by setting the `sideEffects` key in the `package.json`. The paths to the
register file are automatically added by the rollup build.

```json
// @cds/core/package.json
{
  "sideEffects": [
    "./button/register.js"
    ...
