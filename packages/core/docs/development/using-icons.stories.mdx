...
```typescript
import { ClarityIcons, successStandardIcon } from '@clr/core/icon';

ClarityIcons.addIcons(successStandardIcon);
```

The icon service and icons are all exported in the `index.ts`, the root
`@clr/core/icon` entry point. This makes it easy to use but breaks certain performance
...
optimization, and users of the icons should continue to use the root `@clr/core/icon` import.
