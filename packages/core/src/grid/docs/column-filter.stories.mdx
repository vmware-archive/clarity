...
<Meta title="Data Grid (Preview)/Column Filter" />
...
To use column filtering leverage the `cds-search` or `cds-datalist` components. With `cds-action`
...
<cds-grid>
  <cds-grid-column>
    Host <cds-action id="id-filter-demo" shape="filter" aria-label="search available hosts"></cds-action>
  </cds-grid-column>
