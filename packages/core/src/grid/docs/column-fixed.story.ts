...
export function columnFixed() {
  ...
  return html` <cds-grid aria-label="column fixed datagrid demo" height="360">
    ${grid.columns.map(
      (column, i) => html` <cds-grid-column
        resizable
        .position=${i === 0 || i === 4 ? 'fixed' : ''}
...
export function columnFixedDynamic() {
  ...
  class DemoColumnFixedDyanmic extends LitElement {
    ...
    render() {
      return html` <cds-grid aria-label="columns fixed dynamic datagrid demo" height="360">
        <cds-grid-column resizable .position=${this.pinFirst ? 'fixed' : ''} width="150">
          ${this.firstColumn.label}
          <cds-button-action
            @click=${() => (this.pinFirst = !this.pinFirst)}
            .pressed=${this.pinFirst}
            shape=${this.pinFirst ? 'pin' : 'unpin'}
            aria-label="pin ${this.firstColumn.label} column"
          ></cds-button-action>
        </cds-grid-column>
        ...
        <cds-grid-column width="150" resizable .position=${this.pinLast ? 'fixed' : ''}>
          ${this.lastColumn.label}
          <cds-button-action
            @click=${() => (this.pinLast = !this.pinLast)}
            .pressed=${this.pinLast}
            shape=${this.pinLast ? 'pin' : 'unpin'}
            aria-label="pin ${this.lastColumn.label} column"
          ></cds-button-action>
        </cds-grid-column>
        ${this.grid.rows.map(
          row => html` <cds-grid-row>
...
export function columnMultiFixed() {
  const grid = DemoService.data.grid;
  return html` <cds-grid aria-label="column multi fixed datagrid demo" height="360">
    ${grid.columns.map(
      (column, i) => html` <cds-grid-column resizable .position=${i < 2 ? 'fixed' : ''} .width=${i < 2 ? '150' : '300'}
