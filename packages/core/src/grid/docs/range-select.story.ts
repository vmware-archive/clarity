...
export function rangeSelect() {
  ...
  class DemoSelectableCells extends LitElement {
    ...
    render() {
      return html`
        <cds-grid
          aria-label="range selection datagrid demo"
          @rangeSelectionChange=${(e: any) => (this.activeCells = e.detail)}
          height="490"
        >
          ${this.grid.columns.map(column => html`<cds-grid-column>${column.label}</cds-grid-column>`)}
          ${this.grid.rows.map(
            row => html` <cds-grid-row id=${row.id}>
          ...
          <cds-grid-footer>
            <p cds-text="body">
              ...
              ${(
                this.activeCells
                  .filter(c => c.ariaColIndex === '4')
                  .reduce(
                    (prev, cell) =>
                      prev + (this.grid.rows.find(i => i.id === cell.parentElement.id).cells[3].value as number),
                    0
              ...
              ${(
                this.activeCells
                  .filter(c => c.ariaColIndex === '5')
                  .reduce(
                    (prev, cell) =>
                      prev + (this.grid.rows.find(i => i.id === cell.parentElement.id).cells[4].value as number),
                    0
                  ) / (this.activeCells.length ? this.activeCells.length : 1)
              ).toFixed(2)}
            </p>
        ...
        <p cds-text="body">
          Active Cells: ${this.activeCells.map(c => html`(${c.ariaColIndex},${c.parentElement.ariaRowIndex}) `)}
        </p>
      `;
    }
  }
  ...
  class DemoRangeSelectContextMenu extends LitElement {
    ...
    render() {
      return html` <cds-grid
          aria-label="range select context menu datagrid demo"
          @rangeSelectionChange=${(e: any) => (this.activeCells = e.detail)}
          height="490"
        >
          ${this.grid.columns.map(column => html`<cds-grid-column>${column.label}</cds-grid-column>`)}
    ...
    private log() {
      const columns = Array.from(new Set(this.activeCells.map(c => parseInt(c.ariaColIndex) - 1))).map(
        i => this.grid.columns[i].label
      );
      const cells = groupArray(
        this.activeCells.map(c => c.textContent),
        columns.length
      );
