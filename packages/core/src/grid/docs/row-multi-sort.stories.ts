...
export function rowMultiSort() {
  ...
  class DemoRowMultiSort extends LitElement {
    ...
    render() {
      return html`
        <p aria-live="assertive" role="log" cds-text="body">Sort by ${this.grid.columns.filter(c => c.sort !== 'none').map(c => `${c.label} ${c.sort}`).join(' then by ')}</p>
        <cds-grid aria-label="row multi sort datagrid demo" height="360">
          ${this.grid.columns.map((column) => html`<cds-grid-column>
            ${column.label}
            <cds-action-sort aria-label="sort" .sort=${column.sort} @sortChange=${(e: any) => this.sort(column, e.detail)}></cds-action-sort>
          </cds-grid-column>`)}
          ${this.sortedRows.map(row => html`
          <cds-grid-row>
    ...
    private sort(column: DemoGridColumn, sort: ActionSort) {
      column.sort = sort;
