...
describe('cds-grid-placeholder', () => {
  ...
  it('should show default message', async () => {
    await componentIsStable(component);
    expect(component.shadowRoot.querySelector('[part=placeholder]').textContent.trim()).toBe('No Results Found');
  });
  ...
  it('should show drop target message when draggable', async () => {
    ...
    await componentIsStable(component);
    expect(component.shadowRoot.querySelector('[part=placeholder]').textContent.trim()).toBe('Drop Item');
  });
});
