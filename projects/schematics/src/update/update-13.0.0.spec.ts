...
describe('Migration to version 13.0', () => {
  ...
  it('should add the @cds/core package to the project dependencies', async () => {
    ...
    const tree = await runner.runSchematicAsync('migration-13.0', {}, workspaceTree).toPromise();
    expect(tree.files).toContain(packageJsonPath);
    const { dependencies } = JSON.parse(getFileContent(tree, packageJsonPath));

    expect(dependencies).toBeDefined();
    expect(dependencies[clarityCorePackageName]).toEqual('^5.7.0');
  });
