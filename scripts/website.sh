...
[ "$GITHUB_REF" == "refs/head/v5" ] && 
# Deploy to production
node -r dotenv/config -- ./node_modules/.bin/netlify deploy --dir=./dist/website --functions dist/netlify --message="$GITHUB_REF@$GITHUB_SHA" --prod || 
# Deploy a preview
node -r dotenv/config -- ./node_modules/.bin/netlify deploy --dir=./dist/website --functions dist/netlify --message="$GITHUB_REF@$GITHUB_SHA"