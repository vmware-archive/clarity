...
import {Directive, ElementRef, Renderer, AfterViewInit, Output, EventEmitter, OnDestroy} from "@angular/core";
...
export class DatagridColumnResizer implements AfterViewInit, OnDestroy {
    ...
    dragStartHandler(): void {
        this.renderer.setElementStyle(this.handleTrackerEl, "display", "block");
        this.renderer.setElementStyle(document.body, "cursor", "col-resize");
    ...
    dragMoveHandler(moveEvent: any): void {
        ...
        this.renderer.setElementStyle(this.handleTrackerEl, "right", -1 * this.dragDistancePositionX + "px");
    }

    dragEndHandler(): void {
        this.renderer.setElementStyle(this.handleTrackerEl, "right", "0px");
        this.renderer.setElementStyle(this.handleTrackerEl, "display", "none");
        this.renderer.setElementStyle(document.body, "cursor", "auto");
        ...
        if (draggedDistance < 0) {
            if (Math.abs(draggedDistance) < this.columnRectWidth - this.columnMinWidth) {
                if (this.dragWithinMinWidth) {
                    ...
                    this.renderer.setElementClass(this.handleTrackerEl, "exceeded-max", false);
                }
                return draggedDistance;
            } else {
                if (!this.dragWithinMinWidth) {
                    this.dragWithinMinWidth = true;
                    this.renderer.setElementClass(this.handleTrackerEl, "exceeded-max", true);
                }
