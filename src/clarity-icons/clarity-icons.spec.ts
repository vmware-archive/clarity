...
describe("ClarityIcons", () => {
  ...
  describe("ClarityIconsApi.get()", () => {
    ...
    it("should return all icons when no argument is passed in", () => {
      ...
      let currentAllShapes =
          Object.assign({}, CoreShapes, EssentialShapes, BasicShapes,
                        SocialShapes, TechnologyShapes);
    ...
    it("should return CoreShapes['success'] when 'success' is passed in",
       () => {
         ...
         expect(expected).toEqual(removeWhitespace(successIcon));
       });
    ...
    it("should return EssentialShapes['pencil'] when 'pencil' is passed in after including EssentialShapes",
       () => {
         ...
         const expected = `
            ...
            `;
         expect(removeWhitespace(expected))
             .toEqual(removeWhitespace(ClarityIcons.get("pencil")));
       });

    it("should throw an error if the requested shape doesn't exist", () => {
      let nonExistingShape = "non-existing-icon";
      let expectedErrorMessage =
          `'${nonExistingShape}' is not found in the Clarity Icons set.`;
  ...
  describe("ClarityIconsApi.add()", () => {
    ...
    it("should throw an error if an empty string is set for a shape name.",
       () => {
         let expectedErrorMessage =
             `Shape name or alias must be a non-empty string!`;
         expect(() => {
           ClarityIcons.add({"" : ""});
         }).toThrowError(expectedErrorMessage);
       });
    ...
    it("should throw an error if a shape name contains a white space.", () => {
      let expectedErrorMessage =
          `Shape name or alias must not contain any whitespace characters!`;
      expect(() => {
    ...
    it("should add a new shape if a new shape name and template is passed in",
       () => {
         ...
         expect(currentShapeNumber)
             .toBe(Object.keys(ClarityIcons.get()).length - 1);
    ...
    it("should add new shapes if multiple shape names and templates are passed in one by one",
       () => {
         ...
         expect(currentShapeNumber)
             .toBe(Object.keys(ClarityIcons.get()).length - 3);
    ...
    it("should allow override", () => {
      ...
      // Even though ClarityIcons.add() is called twice, only one more property
      // should be added in  as the former one is replaced by the latter one.
  ...
  describe("ClarityIconsApi.alias()", () => {
    ...
    it("should throw an error if the shape name doesn't exist", () => {
      ...
      let expectedErrorMessage =
          "The icon '" + shapeName +
          "' you are trying to set an alias to doesn't exist!";
    ...
    it("should allow aliases if the shape name exists", () => {
      ...
      expect(currentShapeNumber)
          .toBe(Object.keys(ClarityIcons.get()).length - 2);
    ...
    it("should allow to create a new shape by overriding existing alias name",
       () => {
         ...
         let currentShapeNumber = Object.keys(ClarityIcons.get()).length;
         let shapeTemplateOverrideAlias =
             "<svg><title>shape template override alias</title></svg>";
         ClarityIcons.add({"success-mark" : shapeTemplateOverrideAlias});

         // Even though ClarityIcons.add() is called once, no new shapes should
         // be added in  as the existing icons's template is replaced by a new
         // template only.

         expect(currentShapeNumber)
             .toBe(Object.keys(ClarityIcons.get()).length);
         expect(ClarityIcons.get("success-mark"))
             .toBe(shapeTemplateOverrideAlias);
       });
  });
