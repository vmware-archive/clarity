...
export class ClrRangeContainer implements DynamicWrapper, OnDestroy {
  ...
  getRangeProgressFillWidth(): string {
    let input = <HTMLInputElement>this.renderer.selectRootElement('[clrRange]#' + this.idService.id);

    let inputWidth = input.offsetWidth;
    let inputMinValue = +input.min;
    ...
    let inputMiddle = (inputMinValue + inputMaxValue) / 2;
    let inputValue = !!this.control && this.control.value !== undefined ? this.control.value : inputMiddle;
    let valueAsPercent = (inputValue - inputMinValue) * 100 / (inputMaxValue - inputMinValue);
