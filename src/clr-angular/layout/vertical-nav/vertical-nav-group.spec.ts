...
export default function(): void {
    describe("Vertical Nav Group", () => {
        ...
        describe("Nav Group Internals", () => {
            ...
            it("contains the links even if the nav group is not in " +
                   "expanded state when clrIfExpanded is not used",
               () => {
                   expect(navGroup.expanded).toBe(false);
        ...
        describe("Nav Group Internals with clrIfExpanded", () => {
            ...
            it("does not contains the links if the nav group is not in " +
                   "expanded state when clrIfExpanded is used",
               () => {
                   expect(navGroup.expanded).toBe(false);
                   expect(compiled.querySelector(".nav-group-children").children.length).toBe(0);
               });

            it("contains the links if the nav group is not in " +
                   "expanded state when clrIfExpanded is used",
               () => {
                   expect(navGroup.expanded).toBe(false);
                   expect(compiled.querySelector(".nav-group-children").children.length).toBe(0);
